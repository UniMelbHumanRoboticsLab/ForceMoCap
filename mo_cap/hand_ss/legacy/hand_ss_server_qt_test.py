...
class SSTcpReader(QObject):
    ...
    def updateResponse(self):
        ...
        self.msg_ready.emit([json_string])
        # restart the message with the leftovers
        self.buffer_message = self.buffer_message[json_end+2:] 

    def start_worker(self):
        self.reconnect()
        self.ss_response_timer = QTimer()
        self.ss_response_timer.setTimerType(Qt.PreciseTimer)
        self.ss_response_timer.timeout.connect(self.updateResponse)
        self.ss_response_timer.start(5)
    
    @Slot()
    def close(self):
        self.ss_response_timer.stop()
...
class SSHandClient(QObject):
    ...
    def __init__(self, ip="127.0.0.1", port=9004,performer_Id="JQ",side="left",finger_len=1):
        ...
        T = np.eye(4,4)
        if self.side == "left":
            self.vive_to_hand_rot = SE3.Ry(-90, 'deg') * SE3.Rz(-90, 'deg') 
        ...
        # init force response
        self.force_response = np.array([1,1,1,1,1,1,1,1,1])
    ...
    def read_fingers_data(self):
        # read Glove Data
        try:
            self.json_data = json.loads(self.json_response)  # Convert to dictionary
        except Exception as e:
            self.hand_cur_time = self.hand_timer.elapsed()
            self.hand_last_time = self.hand_timer.elapsed()
            pass
            ...
            # process print text and force vector
            self.force_vecs = []
            ...
            for i,distal in enumerate([20,21,22,23,24,16,2,17,9]):
                ...
                self.force_vecs.append(force_pos_vector)
    ...
    def start_worker(self):
        ...
        self.reader_worker = SSTcpReader(self.ip, self.port)
        ...
        self.poll_timer.start(int(1/120*1000)) # run timer at higher frequency than the glove since we cannot refresh TCP to get latest message
    ...
    def update_response(self,worker_response):
         self.json_response = worker_response[0]
    @Slot()
    def stop(self):

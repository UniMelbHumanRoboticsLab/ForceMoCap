        ...
        try:
            ...
            for i,tracker in enumerate(self.trackers):
                ...
                if i == 0: # first tracker is the new inert
                    ...
                    pos_inert = cur_point.t
                    R_inert = R.from_matrix(cur_point.R) # correct the orientation to inertial frame
                    quat_inert = R_inert.as_quat()

                print_text += f"Marker {name}: {pos_inert}\n"

                if name == "LHR-CB87E404" or name == "LHR-125EC9C8":
                    wrist_pos = pos_inert+R_inert.as_matrix()@np.array([0,-15/1000,51/1000])
                    wrists_pos.append(wrist_pos)
                    wrists_frame.append(quat_inert)

...
def convert_all(pose_mat):
    ...
    # Do permutations to adjust the trackers from whatever fkin coordinate system this guy is using
    permute = np.array([[1,0,0],[0,0,1],[0,-1,0]])
    R_tot = np.matmul(permute.T,np.matmul(T.R,permute))
    permute = np.array([[1,0,0],[0,0,-1],[0,1,0]])
    t = np.matmul(permute,t)
    T_rh = SE3.Rt(R_tot,t) # right hand conversion
    ...
    # print(M@T.A@M)
    # assert 0
    quaternion = R.from_matrix(T_rh.R).as_quat().tolist()

    # return position, yaw-pitch-roll (ZYX intrinsic), quaternion and rotation matric
    return [x,y,z],[yaw,pitch,roll],quaternion,T_rh

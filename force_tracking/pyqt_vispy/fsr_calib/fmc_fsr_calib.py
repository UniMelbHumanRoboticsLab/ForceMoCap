...
class FMCCalibFSR(FMCBase):
    ...
    def init_esp_sensor(self):
        ...
        for side,port in zip(sides,ports):
            ...
            if self.force_gui["on?"] and self.force_gui["ESP"]:
                force_module["live_stream_plots"] = self.add_live_stream_plot(live_stream=self.force_mag_live_stream,sensor_name= f"ESP",unit="N",dim=1)
    ...
    def init_rft_sensor(self):
        ...
        if self.force_gui["on?"] and self.force_gui["RFT"]:
            ...
            self.rft_tau_live_stream_plot = self.add_live_stream_plot(live_stream=self.force_mag_live_stream,sensor_name= f"RFT_Tau",unit="Nm",dim=3)
    ...
    def update_gui(self):
        ...
        if hasattr(self, 'esp_response'):
            ...
            self.update_response_label(force_module["response_label"],f'FPS:{fps}\n{raw_data[self.init_args["finger_id"]]}')
            if self.force_gui["on?"] and self.force_gui["ESP"]:
                # update live stream plot
                self.update_live_stream_plot(self.force_mag_live_stream,force_module["live_stream_plots"],np.array([raw_data[self.init_args["finger_id"]]]),dim=1)
